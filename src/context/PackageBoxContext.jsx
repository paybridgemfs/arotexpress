...
export function PackageBoxProvider({ children }) {
  ...
  const loadPackageOrderItems = useCallback(async (items, allPackageProducts = []) => {
    ...
    let pkgProducts = Array.isArray(allPackageProducts) && allPackageProducts.length > 0 ? allPackageProducts : [];
    ...
    if (pkgProducts.length === 0) {
      try {
        ...
        console.warn('Failed to fetch package products for reorder matching:', err);
      }
    }
    ...
    parsedItems.forEach((item) => {
      ...
      if (pkgProducts.length > 0) {
        matchedPkgProduct = pkgProducts.find((p) => {
          ...
          const itPkgId = item.packageProductId ? String(item.packageProductId) : (item.package_product_id ? String(item.package_product_id) : null);
          const itProdId = item.productId ? String(item.productId) : (item.product_id ? String(item.product_id) : (item.id ? String(item.id) : null));
          ...
          // 2. Slot number match
          if (item.slot_number && p.slot_number === item.slot_number) return true;

          // 3. Name match
          const pName = (p.product_name || '').trim().toLowerCase();
          const itName = (item.product_name || item.brand || item.name || '').trim().toLowerCase();
      ...
      if (matchedPkgProduct) {
        newQuantities[matchedPkgProduct.id] = (newQuantities[matchedPkgProduct.id] || 0) + qty;
      } else {
        const fallbackKey = item.packageProductId || item.package_product_id || item.productId || item.product_id || item.id || item.slot_number;
        if (fallbackKey) {
          newQuantities[fallbackKey] = (newQuantities[fallbackKey] || 0) + qty;
        }
      }
    });
